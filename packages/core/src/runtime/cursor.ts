...
export class ExecutionCursor {
  ...
  constructor(cursor: Cursor[] = [[0,0,0]]) {
    if (!cursor.length) {
   ...
   */
  static move(cursor: ExecutionCursor, location: Cursor): ExecutionCursor {
  ...
  static parse(cursorPath: string): ExecutionCursor {
    ...
    const cursor = cursorPath
      .replace(/^\//, '')
      .split('/')
      .map(c => c.split('.').map(Number)) as Cursor[]
...
/**
 * Represents a cursor position in a workflow execution.
